...
with col1:
    ...
    모든 장르가 동일한 시장 환경을 보이는 것은 아닙니다. 일부 장르는 높은 경쟁률을 보이는 레드오션인 반면, 상대적으로 경쟁이 적으면서도 높은 사용자 평가를 받는 장르도 존재합니다. 
    
    본 연구는 Steam 데이터를 활용하여 시장의 경쟁 정도와 사용자 반응을 분석하고, 전략적으로 유효한 장르를 탐색하는 데 목적이 있습니다.
    """)

    st.header("2. 연구 목적")
    st.markdown("""
    본 연구의 목적은 Steam 게임 데이터를 분석하여 경쟁이 과도하게 집중된 장르와 그렇지 않은 장르를 식별하고, 높은 평가를 받는 장르의 핵심 특성을 파악하는 것입니다. 
    
    이를 통해 게임 시장 내에서 새로운 기회를 창출할 수 있는 블루오션 장르를 발굴하고자 합니다.
    """)
