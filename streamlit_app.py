...
st.title("Steam 게임 시장 데이터를 이용한 장르 분석")
...
st.header("2. 연구 목적")
st.markdown("""
본 연구의 목적은 Steam 게임 장르별 빈도와 리뷰 데이터를 활용하여 군집 정보를 파악합니다.
...
- 태그별 경쟁 밀도와 태그별 기준치 이상 게임 비중을 이용하여 장르별 군집 식별
""")
...
with st.expander("포지셔닝 맵 주관적 해석 가이드"):
    st.write("""
    * **고빈도-고비중 (우상단):** 시장의 주류이자 성공 법칙이 검증된 **'스테디셀러 추정 영역'**.
    * **저빈도-고비중 (좌상단):** 경쟁은 적지만 성과는 높은 **'블루오션 추정 기회 영역'**.
    * **고빈도-저비중 (우하단):** 경쟁은 치열하나 성과를 내기 힘든 **'레드오션 추정 영역'**.
    * **저빈도-저비중 (좌하단):** 유저 반응이 아직 확인되지 않은 **'실험 추정 영역'**.
    """)
...
st.write("""
    우측은 시장 경쟁이 치열한 영역을, 상단은 사용자 평가가 우수한 영역을 나타냅니다.
    """)
st.write("""
